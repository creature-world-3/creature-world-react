...
export default function ShopTab({ gs, setGs }) {
  ...
  const doTestEvent = () => {
    ...
    showToast('테스트 이벤트! 도토리 +100개 획득!');
  };

  const doWorldcupDraw = () => {
    if (tickets < WORLDCUP_COST) { showToast(`도토리가 부족해요! ${WORLDCUP_COST}개가 필요해요`); return; }
  ...
  const doWesternDraw = () => {
    if (tickets < WESTERN_COST) { showToast(`도토리가 부족해요! ${WESTERN_COST}개가 필요해요`); return; }
    ...
    <>
      ...
      <div className="shop-wrap">
        <div className="shop-header">
          ...
          <div className="col-count">보유 도토리 {tickets}개</div>
        </div>
        ...
        <div className="shop-item shop-item-western">
          ...
          <div className="shop-item-right">
            ...
            <div className="shop-item-price">도토리 {WESTERN_COST}개</div>
            <button
              className="shop-buy-btn shop-buy-btn-western"
              onClick={doWesternDraw}
              disabled={tickets < WESTERN_COST}
            >
              {tickets < WESTERN_COST ? `도토리 부족 (${tickets}/${WESTERN_COST})` : `뽑기 (${WESTERN_COST}개)`}
            </button>
          </div>
        </div>
        ...
        <div className="shop-item shop-item-western">
          ...
          <div className="shop-item-right">
            ...
            <div className="shop-item-price">도토리 {WORLDCUP_COST}개</div>
            <button
              className="shop-buy-btn shop-buy-btn-western"
              onClick={doWorldcupDraw}
              disabled={tickets < WORLDCUP_COST}
            >
              {tickets < WORLDCUP_COST ? `도토리 부족 (${tickets}/${WORLDCUP_COST})` : `뽑기 (${WORLDCUP_COST}개)`}
            </button>
          </div>
        </div>
        ...
        <div className="shop-item shop-item-event">
          ...
          <div className="shop-item-right">
            ...
            <div className="shop-item-desc">테스트 서버 한정! 도토리 100개를 드려요. 하루 1회.</div>
